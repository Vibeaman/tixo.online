...
export default function EventDetail() {
  ...
  const [userTicketApproved, setUserTicketApproved] = useState(false)
  ...
  useEffect(() => {
    async function checkApproval() {
      if (!user || !event) return
      try {
        const ticket = await TicketService.getUserTicket(event.id, user.id)
        if (ticket?.approved) setUserTicketApproved(true)
  ...
  const isApprovedPrivate = isPrivateVirtual && userTicketApproved
